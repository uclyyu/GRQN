...
def get_binary_weight(img):
    total_count = np.prod(img.shape)
    one_count = img.sum()
    ...
    a = total_count / (2 * one_count)
    b = total_count / (2 * zero_count)
    return a, b
...
def task(basedir, mapsize, nrays, Z, camwh, scene, los):
    ...
    for i in range(5):
        ...
        file_weight = 'ctx-weight-jlos-{:02d}.npy'.format(i)
        ...
        np.save(os.path.join(basedir, file_weight), weight)

        df_ctx.loc[i] = [*scene.normalize_position(ctx_pos), *ctx_orn,
                         file_pxl, file_dep, file_los, file_jlos, file_weight]
        ...
        file_weight = 'qry-weight-los-{:02d}.npy'.format(i)
        ...
        df_qry.loc[i] = [*scene.normalize_position(qry_pos), *qry_orn,
                         file_pxl, file_dep, file_los, file_weight]
