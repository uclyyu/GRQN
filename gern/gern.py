...
class GeRN(torch.jit.ScriptModule):
    def __init__(self, nr=256, nq=64, nh=64, nv=4):
        ...
        self.iop_jstate = RecurrentCell(nr + nq + nh, nh)

        self.iop_dencoder = EncoderD()
        self.iop_dposterior = GaussianFactor()
        self.iop_dstate = RecurrentCell(nr + nq + nh, nh)

        # --- generation operators
        self.gop_jprior = GaussianFactor()
        self.gop_jstate = RecurrentCell(nr + nh, nh)

        self.gop_dprior = GaussianFactor()
        self.gop_dstate = RecurrentCell(nr + nh + nv, nh)
